...
@Route(path = "/apply/extension/12", method = {HttpMethod.PUT})
public class ApplyExtension12 implements Handler<RoutingContext> {
    @Override
    public void handle(RoutingContext ctx) {
        ...
        try {
            ...
            if (!ApplyDataUtil.canApplyExtension12()) {
                ...
                DB.executeUpdate("DELETE FROM extension_apply_12 WHERE uid=?", uid);
                DB.executeUpdate("INSERT INTO extension_apply_12(class, seat, name, uid) VALUES(?, ?, ?, ?)", classId, seatId, name, uid);
