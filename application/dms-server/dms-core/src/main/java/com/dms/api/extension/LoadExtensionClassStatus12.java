...
@Route(path = "/apply/extension/class/12", method = { HttpMethod.GET })
public class LoadExtensionClassStatus12 implements Handler<RoutingContext> {
	@Override
	...
	private HashMap<Integer, String> getSeatDatas(int classId) throws SQLException {
		...
		SafeResultSet resultSet = DataBase.getInstance().executeQuery("SELECT * FROM extension_apply_12 WHERE class=", classId);
